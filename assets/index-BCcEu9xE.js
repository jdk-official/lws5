...
`,b=`<section id="checklist" class="section">
...
`,y=`<section id="coffee" class="section">
...
`,k=`<section id="banks" class="section">
...
`,w=`<section id="trains" class="section">
...
`,h={overview:m,checklist:b,coffee:y,banks:k,trains:w};function x(i){const n=document.getElementById("app"),a=document.querySelectorAll("#main-nav button");function e(t){const r=h[t]??h.overview;n.innerHTML=r,a.forEach(l=>{const c=l.dataset.target===t;l.classList.toggle("active",c),l.setAttribute("aria-selected",String(c))}),i(t,n)}function s(t){const r=document;r.startViewTransition?r.startViewTransition(()=>e(t)):e(t),window.scrollTo({top:0,behavior:"smooth"})}a.forEach(t=>{t.addEventListener("click",()=>{const r=t.dataset.target;r&&s(r)})}),e("overview")}const p="lws5_";function C(i){const n=i.querySelectorAll(".checklist-row");n.forEach(e=>{const s=e.dataset.id;if(!s)return;const t=e.querySelector("input[type=checkbox]");if(!t)return;localStorage.getItem(p+s)==="true"&&(t.checked=!0,e.classList.add("checked"));const l=()=>{t.checked=!t.checked,g(s,t.checked,e)};e.addEventListener("click",c=>{c.target===t||c.target.tagName==="LABEL"||l()}),t.addEventListener("change",()=>g(s,t.checked,e))});const a=i.querySelector("#reset-checklist");a&&a.addEventListener("click",()=>{n.forEach(e=>{const s=e.dataset.id,t=e.querySelector("input[type=checkbox]");!t||!s||(t.checked=!1,e.classList.remove("checked"),localStorage.removeItem(p+s))})})}function g(i,n,a){a.classList.toggle("checked",n),localStorage.setItem(p+i,String(n))}function S(i){const n=i.querySelectorAll(".inner-tab");n.length!==0&&n.forEach(a=>{a.addEventListener("click",()=>{const e=a.dataset.inner;e&&(n.forEach(s=>s.classList.toggle("active",s===a)),i.querySelectorAll("[data-inner-panel]").forEach(s=>{const t=s.dataset.innerPanel===e;s.hidden=!t}))})})}function T(){if(window.matchMedia("(prefers-reduced-motion: reduce)").matches)return;const i=Array.from(document.querySelectorAll(".parallax"));if(i.length===0)return;let n=!1;function a(){const s=window.scrollY;for(const t of i){const r=parseFloat(t.dataset.speed??"0.5"),l=s*(1-r);t.style.transform=`translate3d(0, ${l.toFixed(1)}px, 0)`}n=!1}function e(){n||(n=!0,requestAnimationFrame(a))}window.addEventListener("scroll",e,{passive:!0}),a()}function E(){const i=document.getElementById("progress-bar");if(!i)return;let n=!1;function a(){const e=document.documentElement,s=e.scrollHeight-e.clientHeight,t=s>0?e.scrollTop/s*100:0;i.style.width=t+"%",n=!1}window.addEventListener("scroll",()=>{n||(n=!0,requestAnimationFrame(a))},{passive:!0})}function L(i,n,a){var e;if(i instanceof Element)return[i];if(typeof i=="string"){let s=document;const t=(e=void 0)!==null&&e!==void 0?e:s.querySelectorAll(i);return t?Array.from(t):[]}return Array.from(i)}const P={some:0,all:1};function f(i,n,{root:a,margin:e,amount:s="some"}={}){const t=L(i),r=new WeakMap,l=d=>{d.forEach(o=>{const v=r.get(o.target);if(o.isIntersecting!==!!v)if(o.isIntersecting){const u=n(o);typeof u=="function"?r.set(o.target,u):c.unobserve(o.target)}else typeof v=="function"&&(v(o),r.delete(o.target))})},c=new IntersectionObserver(l,{root:a,rootMargin:e,threshold:typeof s=="number"?s:P[s]});return t.forEach(d=>c.observe(d)),()=>c.disconnect()}function I(i){if(window.matchMedia("(prefers-reduced-motion: reduce)").matches){i.querySelectorAll(".reveal").forEach(e=>e.classList.add("in-view"));return}i.querySelectorAll(".reveal").forEach((e,s)=>{e.style.transitionDelay=`${Math.min(s*.05,.3)}s`,f(e,()=>{e.classList.add("in-view")},{amount:.15})}),i.querySelectorAll(".recipe-card").forEach((e,s)=>{e.style.opacity="0",e.style.transform="translateY(16px)",e.style.transition=`opacity .45s cubic-bezier(.2,.7,.2,1) ${s*.04}s, transform .45s cubic-bezier(.2,.7,.2,1) ${s*.04}s`,f(e,()=>{e.style.opacity="1",e.style.transform="translateY(0)"},{amount:.2})})}E();T();x((i,n)=>{C(n),S(n),I(n)});
//# sourceMappingURL=index-BCcEu9xE.js.map
